...
it('deprecates and persists a blueprint through the application layer', function () {
    ...
    $deprecate = new DeprecateBlueprint($repository);

    $deprecated = $deprecate->handle(
        blueprintId: (string) $blueprint->id(),
    );

    expect($deprecated->lifecycleStatus())
        ->toBe(
            \App\Domain\Blueprint\Enums\LifecycleStatus::DEPRECATED
        );
